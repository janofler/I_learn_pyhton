...
class Particle(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        self.lifetime = 0
        self.maxtime = 1 + random.random() * 4
        self.image = pygame.Surface((4,4))
        pygame.draw.rect(self.image, (255,0,0), (0,0,4,4,)) 
...
def draw_examples(background):
    ...
    pygame.draw.line(background, (255,0,0), (70,70), (150,150))
...
class PygView(object):
    ...
    def __init__(self, width=640, height=400, fps=90):
    ...
    def run(self):
        ...
        self.history = []
        while running:
            self.history.insert(0, self.p1)
            if len(self.history) > 500:
                self.history = self.history[:500]
                self.p2 = self.history[-1]
            pygame.display.set_caption("sprites: {}    fps: {:.2f}".format(
                                 self.balls, self.clock.get_fps() ))
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_p:
                        x = random.randint(100, PygView.width)
                        y = random.randint(50, PygView.height)
                        for p in range(50):
                            Particle(x,y)
            ...
            pygame.draw.line(self.screen, farbe, self.p1, self.p2)
